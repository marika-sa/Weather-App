...
namespace server.Controllers
{
    ...
    {
        public AstronomyController(IHttpClientFactory httpClientFactory, IConfiguration config) : base(httpClientFactory, config)
        { }

        [HttpGet("{location}")]
        public async Task<IActionResult> GetAstronomy(string location)
        {
            return await ExecuteApiRequest("astronomy", location);
        }
    }
}